...
plt.plot(frame_log.onix_time[valid], np.ones(np.sum(valid)), "o", label="Bonsai frames")
plt.legend(loc="lower left")
plt.xlabel("Time (s)")
plt.show()

plt.hist(
    np.diff(frame_times.onix_time.values) * 1000,
    bins=np.arange(200),
    histtype="step",
    label="Photodiode",
)
plt.hist(
    np.diff(frame_log.onix_time.values) * 1000,
    bins=np.arange(200),
    histtype="step",
    label="Bonsai",
)
