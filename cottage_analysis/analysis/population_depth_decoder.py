...
def downsample_dff(
    ...
    trials_df["depth_labels"] = trials_df.apply(lambda x: np.repeat(x["depth_label"], x["dff_stim_downsample"].shape[0], axis=0), axis=1)
    
    return trials_df


def fit_svm_classifier(X_train, y_train, X_test, y_test, X_val, y_val, class_labels, kernel="linear", Cs=[0.1, 1, 10], gammas=[1, 0.1, 0.01]):       
...
def depth_decoder(trials_df, 
    ...
    trials_df = downsample_dff(trials_df,
    ...
    # Train test split
    for fold, (train_index, test_index) in enumerate(stratified_kfold.split(np.arange(len(depth_label)), depth_label)):
    ...
    # Train val split
    for fold, (train_index, val_index) in enumerate(stratified_kfold.split(np.arange(len(depth_label_train)), depth_label_train)):
