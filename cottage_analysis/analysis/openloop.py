...
def find_zeros_before_ones(arr):
    ...
    if len(changes_01) > 0:
        for idx in changes_01:
            assert (arr[idx] == 1)
            zero_start = closest_smaller_value(changes_10, idx) # find the start of the zero sequence
            if zero_start is None: # if the sequence starts with 0
                zero_start = 0
            zero_idx = np.arange(zero_start, idx)
            zeros.append(zero_idx)
            one_stop = closest_bigger_value(changes_10, idx) # find the end of the one sequence
...
def open_before_closed_trials_df(trials_df):
    ...
    if zeros is not None:
        for openloop_trials, closedloop_trials in zip(zeros, ones):
            trials_df_subset = pd.concat([trials_df.iloc[openloop_trials], trials_df.iloc[closedloop_trials]], ignore_index=True)
