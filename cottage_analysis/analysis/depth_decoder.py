...
def separate_data_into_bins(arr, bin_arr, bins, shape):
    if arr.ndim > 1:
        # it's a trace array
        _, _, bin_no = scipy.stats.binned_statistic(
            x=bin_arr, values=arr[0], statistic="mean", bins=bins, range=None
        )
        arr_all_bins = []
        ...
        # it's a param array
        _, _, bin_no = scipy.stats.binned_statistic(
            x=bin_arr, values=arr, statistic="mean", bins=bins, range=None
        )
        arr_all_bins = []
        for i in range(np.nanmin(bin_no), np.nanmax(bin_no) + 1):
...
def rolling_average_2darray(arr, axis, window):
    result = (
        pd.DataFrame(arr)
        .rolling(window, axis=axis, min_periods=1)
        .mean(skipna=True)
        .values
    )
    return result
...
def plot_depth_tuning_curve(
    ...
    if gaussian_fit:
        if (this_depth == None) or (
            ...
            x = np.log(
                np.repeat(np.array(depth_list), trace_arr_mean_eachtrial.shape[1])
            )
...
def main(project, mouse, session):
    ...
    for protocol in protocols:
        ...
        session_analysis_folder_original = (
            root / project / "Analysis" / mouse / session / protocols[0]
        )
        ...
        speed_arr_smoothed = rolling_average_2darray(
            arr=speed_arr_noblank.reshape(-1, speed_arr_noblank.shape[2]),
            axis=1,
            window=window,
        ).reshape(speed_arr_noblank.shape)
        speed_arr_downsam = downsample_2darray(
            arr=speed_arr_smoothed.reshape(-1, speed_arr_smoothed.shape[2]),
            window=window,
            mode="average",
        ).reshape(speed_arr_smoothed.shape[0], speed_arr_smoothed.shape[1], -1)

        # concat all cells dffs
        all_trace_downsam = np.zeros(
            (
                len(which_rois),
                speed_arr_downsam.shape[0],
                speed_arr_downsam.shape[1],
                speed_arr_downsam.shape[2],
            )
        )

        for iroi, roi in enumerate(which_rois):
            # Load trace_arr
            trace_arr_noblank, _ = get_trace_arrs(
                roi=roi,
                dffs=dffs_ast_all,
            ...
            # Downsample trace
            trace_arr_downsam = downsample_2darray(
                arr=trace_arr_noblank.reshape(-1, trace_arr_noblank.shape[2]),
                window=window,
                mode="average",
            ).reshape(trace_arr_noblank.shape[0], trace_arr_noblank.shape[1], -1)
        ...
        # train test split
        depth_labels = np.repeat(
            np.repeat(np.arange(len(depth_list)), speed_arr_downsam.shape[1]),
            speed_arr_downsam.shape[2],
        ).reshape(
            len(depth_list), speed_arr_downsam.shape[1], speed_arr_downsam.shape[2]
        )
        speed_min = 1
        speed_max = 150
        ...
        (
            ...
            p_test=0.5, xarr1=rs_test, xarr2=trace_test, yarr=depth_test
        )
        ...
        acc_same, conmat_same, clf = train_classifier(
            X_train=trace_train.T,
            X_test=trace_test.T,
            X_val=trace_val.T,
            y_train=depth_train,
            y_test=depth_test,
        ...
        (
            rs_classifier_train_test,
            rs_val,
            trace_classifier_train_test,
            trace_val,
            depth_classifier_train_test,
            depth_val,
        ) = train_test_split_trials(
            p_test=0.1,
            xarr1=rs_classifier,
        ...
        trace_rest, depth_rest, rs_rest = remove_nan(
            xarr=trace_rest, arr1=depth_rest, arr2=rs_rest, shape=(len(which_rois), -1)
        )
        trace_train, depth_train, rs_train = remove_nan(
            xarr=trace_train,
            arr1=depth_train,
            arr2=rs_train,
            shape=(len(which_rois), -1),
        )
        trace_val, depth_val, rs_val = remove_nan(
            xarr=trace_val, arr1=depth_val, arr2=rs_val, shape=(len(which_rois), -1)
        )
        trace_classifier_test, depth_classifier_test, rs_classifier_test = remove_nan(
            xarr=trace_classifier_test,
            arr1=depth_classifier_test,
            arr2=rs_classifier_test,
            shape=(len(which_rois), -1),
        )
        ...
        trace_all_bins = separate_data_into_bins(
            arr=trace_rest, bin_arr=rs_rest, bins=rs_bins, shape=(len(which_rois), -1)
        )
        rs_all_bins = separate_data_into_bins(
            arr=rs_rest, bin_arr=rs_rest, bins=rs_bins, shape=(len(which_rois), -1)
        )
        depth_all_bins = separate_data_into_bins(
            arr=depth_rest, bin_arr=rs_rest, bins=rs_bins, shape=(len(which_rois), -1)
        )
        ...
        for i in range(test_bins):
            ...
            conmat = confusion_matrix(
                depth_all_bins[i], y_pred, labels=np.arange(len(depth_list))
            )
            all_accs.append(acc)
