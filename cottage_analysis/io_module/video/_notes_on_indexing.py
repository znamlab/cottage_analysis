...
print(
    "d[d1,d2,d3] = data[d3 + N3 * d2 + N3 * N2 * d1], where Nx is the shape of dimension x"
)
...
print()
print(
    "We want a C-order (numpy default) array with the pixel information being contiguous on disk"
)
...
check_new_fp = np.memmap(
    newmmp_filename, dtype="float32", mode="r", shape=(Nrows, Ncolumns, Nframes)
)
