...
def create_stim_dict(depth_list, img_VS, choose_trials=None):
    ...
    for istim in depth_list:
        ...
        stim_dict['stim' + str(istim)]['start'] = img_VS_simple[
            (img_VS_simple['Depth'] == istim) & (img_VS_simple['Stim'] == 1)].index.values
        stim_dict['stim' + str(istim)]['stop'] = img_VS_simple[
            (img_VS_simple['Depth'] == istim) & (img_VS_simple['Stim'] == 0)].index.values
        if choose_trials != None:
            stim_dict['stim' + str(istim)]['start'] = stim_dict['stim' + str(istim)]['start'][:choose_trials]
            stim_dict['stim' + str(istim)]['stop'] = stim_dict['stim' + str(istim)]['stop'][:choose_trials]
    ...
    if choose_trials != None:
        stim_dict['blank']['start'] = stim_dict['blank']['start'][:choose_trials * len(depth_list)]
        stim_dict['blank']['stop'] = stim_dict['blank']['stop'][:choose_trials * len(depth_list)]
    ...
    for istim in depth_list:
        stim_dict['stim_all']['start'].append(list(stim_dict['stim' + str(istim)]['start']))
        stim_dict['stim_all']['stop'].append(list(stim_dict['stim' + str(istim)]['stop']))
    stim_dict['stim_all']['start'] = np.sort(np.array([j for i in stim_dict['stim_all']['start'] for j in i]))
    stim_dict['stim_all']['stop'] = np.sort(np.array([j for i in stim_dict['stim_all']['stop'] for j in i]))
    stim_dict['stim_all']['depth'] = np.array(img_VS_simple[::2].Depth)
    if choose_trials != None:
        stim_dict['stim_all']['depth'] = stim_dict['stim_all']['depth'][:choose_trials * len(depth_list)]
...
def find_frame_num_per_trial(depth_list, stim_dict, mode='max', isStim=True):
    ...
    # Blank period
        ...
        frame_num_result = int(np.min(frame_num_arr))  # always find the min frame num for blank period
