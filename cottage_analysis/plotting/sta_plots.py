...
def plot_sta_fit(coef, depth_list, frames, roi, plot_rows=5, plot_cols=1, plot_y=0, plot_idx=1):
    ...
    for i in range(len(depth_list)):
        ...
        plt.imshow(fitted_sta[i,:,:,roi], origin='lower', vmax=vmax, vmin=-vmax, cmap='bwr', extent=[0,120,-40,40])
        plt.colorbar()


def basic_vis_sta(coef, neurons_df, trials_df, depth_list, frames, roi, plot_rows=6, plot_cols=1, plot_y=0, fontsize_dict={"title": 10, "tick": 10, "label": 10}):
    ...
    for i in tqdm(range(len(neurons_df)//10+1)):
        if not i*10 >= len(neurons_df):
            ...
            for roi in np.arange(len(neurons_df))[i*10:max_roi]:
                basic_vis_sta(coef, neurons_df, trials_df, depth_list, frames, roi, plot_rows=len(depth_list)+1, plot_cols=10, plot_y=iroi,  fontsize_dict=fontsize_dict)
                iroi+=1
            plt.savefig(save_dir/"plots"/"sta"/f"roi{np.arange(len(neurons_df))[i*10]}-{np.arange(len(neurons_df))[max_roi]}.png", dpi=100, bbox_inches="tight")