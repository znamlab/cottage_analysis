...
def plot_treadmill_vs_closedloop_matrix(
    ...
    **kwargs,
    ...
    for idx_df, trials_df in enumerate([trials_df_tread, trials_df_sphere]):
        ...
        bin_means, _, _, _ = scipy.stats.binned_statistic_2d(
            x=rs_arr,
            y=of_arr,
            values=dff_arr,
            statistic="mean",
        ...
        vmin = np.nanmin([vmin, np.nanmin(bin_means[1:, 1:].flatten())])
        vmax = np.nanmax([vmax, np.nanmax(bin_means[1:, 1:].flatten())])
    ...
    if split_tread_half:
        ...
        for idx in trials_df_tread.index:
            dff = trials_df_tread.at[idx, "dff_stim"]
            dff_first_half = dff[: len(dff) // 2]
            dff_second_half = dff[len(dff) // 2 :]
            trials_df_tread_first_half.at[idx, "dff_stim"] = dff_first_half
            trials_df_tread_second_half.at[idx, "dff_stim"] = dff_second_half
