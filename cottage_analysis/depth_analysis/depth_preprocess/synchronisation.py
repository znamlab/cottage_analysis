...
def find_monitor_frames(project, mouse, session, protocol, irecording=0, redo=True, redo_harpnpz=False):
...
def generate_trials_df(project, mouse, session, protocol, vs_df, irecording=0):
    ...
    # trials_df
    trials_df = pd.DataFrame(columns=['trial_no','depth', 
                                    'start_time_stim', 'stop_time_stim', 'start_time_blank', 'stop_time_blank',
                                    'start_imaging_frame_stim', 'stop_imaging_frame_stim', 'start_imaging_frame_blank', 'stop_imaging_frame_blank',
                                    'RS_array_stim', 'RS_array_blank', 'RS_eye_array_stim',
                                    'OF_array_stim', 
                                    'spheres_no', 'closed_loop'])
    ...
    if len(start_idx_stim) != len(start_idx_blank):
        ...
        stop_idx_blank = np.append(stop_idx_blank, (np.abs(vs_df['onset_time'] -last_blank_stop_time)).idxmin())
    ...
    trials_df.start_imaging_frame_blank = vs_df.loc[start_idx_blank].imaging_frame.values
    ...
    vs_img_frames['RS_eye'] = vs_img_frames.eye_z.diff()/vs_img_frames.onset_time.diff()
    trials_df.RS_array_stim = trials_df.apply(lambda x: vs_img_frames.RS.loc[x.start_imaging_frame_stim:x.stop_imaging_frame_stim].values, axis=1) # This is not RS, this is mouseZ!!!
    trials_df.RS_array_blank = trials_df.apply(lambda x: vs_img_frames.RS.loc[x.start_imaging_frame_blank:x.stop_imaging_frame_blank].values, axis=1)
    trials_df.RS_eye_array_stim = trials_df.apply(lambda x: vs_img_frames.RS_eye.loc[x.start_imaging_frame_stim:x.stop_imaging_frame_stim].values, axis=1)
    trials_df.OF_array_stim = trials_df.apply(lambda x: x['RS_eye_array_stim']/x['depth'], axis=1)
