...
def main(project, mouse, session, protocol):
    ...
    rawdata_folder, protocol_folder, analysis_folder, suite2p_folder, trace_folder = generate_filepaths.generate_file_folders(project=project,
                                                                                                           mouse=mouse,
                                                                                                           session=session,
                                                                                                           protocol=protocol,
    ...
    if 'Playback' in protocol:
        rawdata_folder_closeloop, protocol_folder_closeloop, analysis_folder_closeloop, suite2p_folder_closeloop, trace_folder_closeloop = generate_filepaths.generate_file_folders(project=project,
                                                                                                           mouse=mouse,
                                                                                                           session=session,
    ...
    # Calculate dF/F
    if calculate_dFF:
        dffs_ast = process_trace.calculate_dFF(f=Fast, mode='gmm', n_components=2, verbose=True)
    ...
    VS_param_file = generate_filepaths.generate_logger_path(project=project,
                                                              mouse=mouse,
                                                              session=session,
                                                              protocol=protocol,
                                                              rawdata_root=rawdata_root,
    ...
    img_frame_logger = format_loggers.format_img_frame_logger(harpmessage_file, register_address=32)
    frame_number = ops['frames_per_folder'][folder_no]
    img_frame_logger = find_frames.find_imaging_frames(harp_message=img_frame_logger, frame_number=frame_number,
                                                           exposure_time=0.0324 * 2, register_address=32,
    ...
    img_VS = pd.merge_asof(img_frame_logger, VS_param_logger, on='HarpTime', allow_exact_matches=True, direction='backward')

    # Add running logger to the aligned dataframe (ALL DISTANCE IN METERS)
    rotary_encoder_file = generate_filepaths.generate_logger_path(project=project,
                                                              mouse=mouse,
                                                              session=session,
                                                              protocol=protocol,
                                                              rawdata_root=rawdata_root,
    ...
    mousez_logger = pd.read_csv(rotary_encoder_file, sep=',', usecols=['HarpTime', 'EyeZ','MouseZ'])
    ...
    speed_arr, _ = create_speed_arr(speeds=speeds, depth_list=depth_list, stim_dict=stim_dict,
    ...
    for roi in which_rois:
        trace_arr, _ = create_trace_arr_per_roi(which_roi=roi, dffs=dffs, depth_list=depth_list, stim_dict=stim_dict,
                                                mode='sort_by_depth', protocol='fix_length',
                                                blank_period=0, frame_rate=frame_rate)
        ...
        _, p = scipy.stats.f_oneway(trace_arr_mean_eachtrial[0], trace_arr_mean_eachtrial[1], trace_arr_mean_eachtrial[2],
                              trace_arr_mean_eachtrial[3], trace_arr_mean_eachtrial[4])
        anova_ps.append(p)
    ...
    depth_neurons = which_rois[np.where(anova_ps < 0.05)[0]]  # ROI no. for depth neurons
    ...
    for iroi in range(len(depth_neurons)):
        ...
        trace_arr, _ = create_trace_arr_per_roi(roi, dffs, depth_list, stim_dict, mode='sort_by_depth',
                                                protocol='fix_length', blank_period=0, frame_rate=15)
        trace_arr_mean_eachtrial = np.nanmean(trace_arr, axis=2)
   ...
   # PLOT DEPTH NEURONS
    ...
    for i in range(len(depth_list)):
        for n in depth_neurons[max_depths==i]:
            ...
            im[ypix,xpix,:] = np.asarray([line_colors[i][0],line_colors[i][1],line_colors[i][2]])
    ...
    plt.vlines(0.05, 0, np.max(patches.datavalues), linestyle='dotted', color='r', linewidth=2)
    ax = plt.gca()
    ...
    for i in range(len(depth_list)):
        if i == 0:
            plt.bar(x='compare mean', height=max_depths_count[i] / len(which_rois) * 100, width=0.5, color=line_colors[i],
                    align='center')
        else:
            plt.bar(x='compare mean', height=max_depths_count[i] / len(which_rois) * 100, width=0.5,
                    bottom=max_depths_count_cumsum[i - 1] / len(which_rois) * 100, color=line_colors[i], align='center')
    plt.bar(x='compare mean', height=max_depths_count[-1] / len(which_rois) * 100, width=0.5,
            bottom=max_depths_count_cumsum[-1] / len(which_rois) * 100, color='gray', align='center')
    plt.xlim([-1, 1])
    ...
    plt.bar(x=np.arange(len(depth_list) + 1), height=np.array(max_depths_count) / len(which_rois) * 100)
