...
import numpy as np
import pandas as pd
from cottage_analysis.stimulus_structure.sphere_structure import find_min_trial_num_all_depths, find_frame_num_per_trial


def create_trace_arr_per_roi(which_roi, dffs, depth_list, stim_dict, mode, protocol='fix_length', isStim=True,
                             blank_period=0, frame_rate=30):
    '''
    ...
    if protocol == 'fix_time':
        ...
        frame_num_pertrial = find_frame_num_per_trial(depth_list, stim_dict, mode='min', isStim=isStim)
        ...
        if isStim:
            if mode == 'sort_by_depth':
                ...
                trace_arr = np.zeros((1, len(stim_dict['stim_all']['start']), frame_num_pertrial))
                for itrial in range(0, len(stim_dict['stim_all']['start'])):
                    frame_start = stim_dict['stim_all']['start'][itrial] - blank_period * frame_rate
        ...
        trial_num = find_min_trial_num_all_depths(stim_dict, depth_list, isStim=isStim)
        frame_num_pertrial_max = find_frame_num_per_trial(depth_list, stim_dict, mode='max', isStim=isStim)
        dff = dffs[which_roi, :]
        ...
        if isStim:
            if mode == 'sort_by_depth':
                ...
                trace_arr = np.zeros(
                    (len(depth_list), trial_num, (frame_num_pertrial_max + blank_period * frame_rate * 2)))
                trace_arr[:] = np.nan
                trace_list = [[[[] for i in range(3)] for j in range(trial_num)] for k in range(len(depth_list))]
                for idepth in range(0, len(depth_list)):
                    depth = depth_list[idepth]
                    for itrial in range(0, trial_num):
                        frame_start = stim_dict['stim' + str(depth)]['start'][itrial]
                        frame_num_this_trial = stim_dict['stim' + str(depth)]['stop'][itrial] - \
                                               stim_dict['stim' + str(depth)]['start'][itrial] + 1
                        trace_arr[idepth, itrial, :int(frame_num_this_trial + blank_period * frame_rate)] = dff[(
                                                                                                                            frame_start - blank_period * frame_rate)
                                                                                                                :(
                                                                                                                            frame_start + frame_num_this_trial)]
                        trace_arr[idepth, itrial, frame_num_pertrial_max + blank_period * frame_rate:] = dff[(
                                                                                                                         frame_start + frame_num_this_trial)
                                                                                                             :(
                                                                                                                         frame_start + frame_num_this_trial + blank_period * frame_rate)]

                        trace_list[idepth][itrial][0] = dff[
                                                        (frame_start - blank_period * frame_rate):frame_start].tolist()
                        trace_list[idepth][itrial][1] = dff[frame_start:(frame_start + frame_num_this_trial)].tolist()
                        trace_list[idepth][itrial][2] = dff[(frame_start + frame_num_this_trial):(
                                    frame_start + frame_num_this_trial + blank_period * frame_rate)].tolist()

            elif mode == 'all_trials':
                trace_arr = np.zeros(
                    (1, trial_num * len(depth_list), (frame_num_pertrial_max + blank_period * frame_rate * 2)))
                trace_arr[:] = np.nan
                trace_list = [[[[] for i in range(3)] for j in range(trial_num * len(depth_list))] for k in range(1)]
                for itrial in range(trial_num * len(depth_list)):
                    frame_start = stim_dict['stim_all']['start'][itrial]
                    frame_num_this_trial = stim_dict['stim_all']['stop'][itrial] - stim_dict['stim_all']['start'][
                        itrial] + 1
                    trace_arr[0, itrial, :int(frame_num_this_trial + blank_period * frame_rate)] = dff[(
                                                                                                                   frame_start - blank_period * frame_rate)
                                                                                                       :(
                                                                                                                   frame_start + frame_num_this_trial)]
                    trace_arr[0, itrial, frame_num_pertrial_max + blank_period * frame_rate:] = dff[(
                                                                                                                frame_start + frame_num_this_trial)
                                                                                                    :(
                                                                                                                frame_start + frame_num_this_trial + blank_period * frame_rate)]

                    trace_list[0][itrial][0] = dff[(frame_start - blank_period * frame_rate):frame_start].tolist()
                    trace_list[0][itrial][1] = dff[frame_start:(frame_start + frame_num_this_trial)].tolist()
                    trace_list[0][itrial][2] = dff[(frame_start + frame_num_this_trial):(
                                frame_start + frame_num_this_trial + blank_period * frame_rate)].tolist()
            ...
            for itrial in range(0, trial_num):
                frame_start = stim_dict['blank']['start'][itrial]
                trace_arr[0, itrial, :] = dff[frame_start:(frame_start + frame_num_pertrial_max)]
                trace_list = trace_arr.tolist()

        return trace_arr, trace_list


def create_speed_arr(speeds, depth_list, stim_dict, mode, protocol='fix_length', isStim=True, blank_period=0,
                     frame_rate=30):
    '''
    ...
    if protocol == 'fix_time':
        ...
        if isStim:
            if mode == 'sort_by_depth':
                ...
                speed_arr = np.zeros((1, len(stim_dict['stim_all']['start']), frame_num_pertrial))
                for itrial in range(0, len(stim_dict['stim_all']['start'])):
                    frame_start = stim_dict['stim_all']['start'][itrial] - blank_period * frame_rate
        ...
        if isStim:
            if mode == 'sort_by_depth':
                ...
                speed_arr = np.zeros(
                    (len(depth_list), trial_num, (frame_num_pertrial_max + blank_period * frame_rate * 2)))
                speed_arr[:] = np.nan
                speed_list = [[[[] for i in range(3)] for j in range(trial_num)] for k in range(len(depth_list))]
                for idepth in range(0, len(depth_list)):
                    depth = depth_list[idepth]
                    for itrial in range(0, trial_num):
                        frame_start = stim_dict['stim' + str(depth)]['start'][itrial]
                        frame_num_this_trial = stim_dict['stim' + str(depth)]['stop'][itrial] - \
                                               stim_dict['stim' + str(depth)]['start'][itrial] + 1
                        speed_arr[idepth, itrial, :int(frame_num_this_trial + blank_period * frame_rate)] = speeds[(
                                                                                                                               frame_start - blank_period * frame_rate)
                                                                                                                   :(
                                                                                                                               frame_start + frame_num_this_trial)]
                        speed_arr[idepth, itrial, frame_num_pertrial_max + blank_period * frame_rate:] = speeds[(
                                                                                                                            frame_start + frame_num_this_trial)
                                                                                                                :(
                                                                                                                            frame_start + frame_num_this_trial + blank_period * frame_rate)]
                        ...
                        speed_list[idepth][itrial][1] = speeds[
                                                        frame_start:(frame_start + frame_num_this_trial)].tolist()
                        speed_list[idepth][itrial][2] = speeds[(frame_start + frame_num_this_trial):(
                                    frame_start + frame_num_this_trial + blank_period * frame_rate)].tolist()
                ...
                speed_arr = np.zeros(
                    (1, trial_num * len(depth_list), (frame_num_pertrial_max + blank_period * frame_rate * 2)))
                speed_arr[:] = np.nan
                speed_list = [[[[] for i in range(3)] for j in range(trial_num * len(depth_list))] for k in range(1)]
                for itrial in range(trial_num * len(depth_list)):
                    frame_start = stim_dict['stim_all']['start'][itrial]
                    frame_num_this_trial = stim_dict['stim_all']['stop'][itrial] - stim_dict['stim_all']['start'][
                        itrial] + 1
                    speed_arr[0, itrial, :int(frame_num_this_trial + blank_period * frame_rate)] = speeds[(
                                                                                                                      frame_start - blank_period * frame_rate)
                                                                                                          :(
                                                                                                                      frame_start + frame_num_this_trial)]
                    speed_arr[0, itrial, frame_num_pertrial_max + blank_period * frame_rate:] = speeds[(
                                                                                                                   frame_start + frame_num_this_trial)
                                                                                                       :(
                                                                                                                   frame_start + frame_num_this_trial + blank_period * frame_rate)]

                    speed_list[0][itrial][0] = speeds[(frame_start - blank_period * frame_rate):frame_start].tolist()
                    speed_list[0][itrial][1] = speeds[frame_start:(frame_start + frame_num_this_trial)].tolist()
                    speed_list[0][itrial][2] = speeds[(frame_start + frame_num_this_trial):(
                                frame_start + frame_num_this_trial + blank_period * frame_rate)].tolist()
            ...
            for itrial in range(0, trial_num):
                frame_start = stim_dict['blank']['start'][itrial]
                speed_arr[0, itrial, :] = speeds[frame_start:(frame_start + frame_num_pertrial_max)]
