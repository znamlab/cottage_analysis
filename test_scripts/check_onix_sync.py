...
if Path(processed_messages).is_file():
    ...
    harp_message = cottage_analysis.io_module.harp.load_harp(
        raw / VIS_STIM / ("%s_%s_%s_harpmessage.bin" % (MOUSE, SESSION, VIS_STIM))
    )
...
### DO SOME CHECKS ###
######################
fig = plot_onix_harp_clock_sync(
    oni_clock_di=oni_clock_di,
